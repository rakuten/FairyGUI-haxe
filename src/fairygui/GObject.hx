...
class GObject extends EventDispatcher
{
    ...
    {
        return try cast(this, GImage) catch(e:Dynamic) null;
    }
    ...
    public function setup_beforeAdd(xml : FastXML) : Void
    {
        ...
        {
            arr = str.split(",");
            _initWidth = Std.parseInt(arr[0]);
            _initHeight = Std.parseInt(arr[1]);
        ...
        {
            arr = str.split(",");
            var n1 : Float = Std.parseFloat(arr[0]);
            var n2 : Float = Std.parseFloat(arr[1]);
            //旧版本的兼容性处理
            if (n1 > 2)
            {
                if (_sourceWidth != 0) 
